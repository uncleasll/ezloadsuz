...
def _recalculate(db: Session, settlement_id: int):
    """
    Recompute settlement_total and balance_due from:
    - items (load payables — use amount_snapshot, the historically frozen value)
    - adjustments (additions add, deductions subtract)
    - payments (always subtract)

    This NEVER touches live driver profile values.
    """
