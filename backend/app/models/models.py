...
class Load(Base):
    ...
    status = Column(SAEnum(LoadStatus), default=LoadStatus.NEW, nullable=False)
    billing_status = Column(SAEnum(BillingStatus), default=BillingStatus.PENDING, nullable=False)
...
class LoadStop(Base):
    ...
    stop_type = Column(SAEnum(StopType), nullable=False)
...
class SettlementStatus(str, enum.Enum):
    PREPARING = "Preparing"
    READY = "Ready"          # "Ready for payment" in UI
    SENT = "Sent"
    PAID = "Paid"
    VOID = "Void"
...
class Settlement(Base):
    ...
    status = Column(SAEnum(SettlementStatus), default=SettlementStatus.PREPARING)
...
class User(Base):
    ...
    role = Column(SAEnum(UserRole), default=UserRole.DISPATCHER, nullable=False)
    is_active = Column(Boolean, default=True)
...
class DriverDocument(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    driver_id = Column(Integer, ForeignKey("drivers.id"), nullable=False)
    doc_type = Column(SAEnum(DriverDocType), nullable=False)
    status = Column(String(100), nullable=True)
    number = Column(String(100), nullable=True)          # CDL/SSN/card number
    state = Column(String(10), nullable=True)             # issuing state
    ...
    name = Column(String(200), nullable=True)             # for 'other' doc type
