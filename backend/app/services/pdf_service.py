...
def _get_company_info(db):
    """DB dan company ma'lumotlarini oladi. company_service yo'q bo'lsa ham ishlaydi."""
    try:
    ...
    # company_service yo'q bo'lsa, modeldan to'g'ridan to'g'ri o'qiymiz
    try:
        from app.models.models import CompanySettings
        co = db.query(CompanySettings).first()
        if co:
            return {
                "name":       getattr(co, "name", "")       or "My Company",
                "email":      getattr(co, "email", "")      or "",
                "phone":      getattr(co, "phone", "")      or "",
                "city":       getattr(co, "city", "")       or "",
                "state":      getattr(co, "state", "")      or "",
                "zip_code":   getattr(co, "zip_code", "")   or "",
                "mc_number":  getattr(co, "mc_number", "")  or "",
                "dot_number": getattr(co, "dot_number", "") or "",
            }
    except Exception:
        pass

    # Hech narsa topilmasa default
    return {
        "name": "My Company", "email": "", "phone": "",
        "city": "", "state": "", "zip_code": "",
...
def generate_settlement_pdf(settlement, db=None) -> bytes:
    ...
    item_dates = [item.load_date for item in (settlement.items or []) if item.load_date]
    ...
    # ── Header ─────────────────────────────────────────────────────────────────
    left_block = [P(payable_to, head_l)]
    if drv_phone: left_block.append(P(drv_phone, sub_l))
    if drv_email: left_block.append(P(drv_email, sub_l))

    right_block = [P(co_name, head_r)]
    if co_email: right_block.append(P(f"Email: {co_email}", sub_r))
    if co_phone: right_block.append(P(f"Phone: {co_phone}", sub_r))

    hdr_tbl = Table([[left_block, right_block]], colWidths=[3.6*inch, 3.6*inch])
    ...
    for item in load_items:
        ...
        pay_type = getattr(load, "pay_type_snapshot", None) or "per_mile"
        amount   = item.amount_snapshot if item.amount_snapshot is not None else item.amount
        subtotal += amount

        if pay_type == "percentage":
            pct     = getattr(load, "freight_percentage_snapshot", 0.0) or 0.0
            formula = f"${load.rate:,.2f} × {pct:.0f}% = ${amount:,.2f}"
        elif pay_type == "flatpay":
            formula = f"Flat pay = ${amount:,.2f}"
        else:
            rate_l  = getattr(load, "pay_rate_loaded_snapshot", 0.65) or 0.65
            rate_e  = getattr(load, "pay_rate_empty_snapshot",  0.30) or 0.30
