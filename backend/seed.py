...
try:
    ...
    # ── Trailers ──────────────────────────────────────────────────────────────
    trailers = [
        Trailer(unit_number="TRL001", trailer_type="Dry Van", length=53, is_active=True),
        Trailer(unit_number="TRL002", trailer_type="Reefer", length=53, is_active=True),
        Trailer(unit_number="TRL003", trailer_type="Dry Van", length=48, is_active=True),
    ]
    db.add_all(trailers)
    db.flush()
    ...
    # ── Loads ─────────────────────────────────────────────────────────────────
    loads_cfg = [
        dict(num=1001, status=LoadStatus.PICKED_UP, billing=BillingStatus.PENDING,
             ...
             services=[dict(type=ServiceType.LUMPER, add="Deduct", inv=213.0, pay=0.0, notes="Advanced")]),

        dict(num=1002, status=LoadStatus.NEW, billing=BillingStatus.PENDING,
        ...
        dict(num=1003, status=LoadStatus.DELIVERED, billing=BillingStatus.INVOICED,
        ...
        dict(num=1004, status=LoadStatus.CLOSED, billing=BillingStatus.PAID,
        ...
        dict(num=1005, status=LoadStatus.EN_ROUTE, billing=BillingStatus.PENDING,
        ...
        dict(num=1006, status=LoadStatus.DISPATCHED, billing=BillingStatus.PENDING,
        ...
        dict(num=1007, status=LoadStatus.DELIVERED, billing=BillingStatus.BOL_RECEIVED,
             ...
             services=[dict(type=ServiceType.DETENTION, add="Add", inv=150.0, pay=75.0, notes="4hr delay")]),

        dict(num=1008, status=LoadStatus.CLOSED, billing=BillingStatus.PAID,
        ...
        dict(num=1009, status=LoadStatus.DELIVERED, billing=BillingStatus.SENT_TO_FACTORING,
        ...
        dict(num=1010, status=LoadStatus.TONU, billing=BillingStatus.CANCELED,
        ...
        dict(num=1011, status=LoadStatus.CLOSED, billing=BillingStatus.PAID,
        ...
        dict(num=1012, status=LoadStatus.DELIVERED, billing=BillingStatus.INVOICED,
             ...
             services=[dict(type=ServiceType.LUMPER, add="Add", inv=250.0, pay=125.0, notes="Unload fee")]),
    ]
    ...
    for cfg in loads_cfg:
        ...
        for i, (stype, city, state, zip_, sdate) in enumerate(cfg["stops"]):
            db.add(
                LoadStop(
                    load_id=load.id,
                    stop_type=StopType.PICKUP if stype == "pickup" else StopType.DELIVERY,
                    stop_order=i + 1,
                    city=city,
                    state=state,
                    zip_code=zip_,
        ...
        db.add(
            LoadHistory(
                ...
                description=(
                    ...
                    f"status: {cfg['status'].value}; billing: {cfg['billing'].value}"
                ),
