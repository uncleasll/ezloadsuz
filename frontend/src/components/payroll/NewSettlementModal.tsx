...
export default function NewSettlementModal({ drivers, onClose, onSaved }: Props) {
  ...
  // Reload whenever driver filter changes
          ...
          <div>
            <div className="flex items-center gap-4 mb-4 flex-wrap">
              ...
              <button onClick={fetchBalances}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-brand-600 hover:bg-brand-700 text-white text-xs font-medium rounded flex-shrink-0">
                <IcoCheck /> Apply
              </button>
            </div>
                  ...
                  {loading ? (
                    ...
                    <tr key={b.driver_id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-gray-500">
                        {/* backend returns 'updated' field, not 'last_load_date' */}
                        {(b as any).updated ? formatDate((b as any).updated) : b.last_load_date ? formatDate(b.last_load_date) : '—'}
                      </td>
